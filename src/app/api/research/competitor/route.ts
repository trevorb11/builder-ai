...
import { researchCompetitor } from "@/lib/ai";
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { competitorName, criteria } = await req.json();
    ...
    const researchCriteria = criteria || [
    ...
    const report = await prisma.deepResearchReport.create({
      data: {
        ...
        status: "in_progress",
        metadata: JSON.stringify({ competitorName, criteria: researchCriteria }),
        startedAt: new Date(),
